...
class SimpleOps(TensorOps):
    ...
    def reduce(
        ...
        ----
            ...
            start (float): float indicating starting value for reduce
...
def tensor_map(
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        output_size = len(out)
        # Create indicies for input and output
        input_index = np.array([0] * len(in_shape), dtype=np.int32)
        output_index = np.array([0] * len(out_shape), dtype=np.int32)
        # Loop through each ordinal of the output
        for ordinal in range(output_size):
...
def tensor_zip(
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        output_size = len(out)
        # Create indicies for input and output
        a_input_index = np.array([0] * len(a_shape), dtype=np.int32)
        b_input_index = np.array([0] * len(b_shape), dtype=np.int32)
        output_index = np.array([0] * len(out_shape), dtype=np.int32)
        # Loop through each ordinal of the output
        for ordinal in range(output_size):
            # Given the ordinal, find its corresponding output index
    ...
    def _reduce(
        ...
        for ordinal in range(output_size):
            ...
            for i in range(a_shape[reduce_dim]):
                # Change input index such that you go through all elements in reduce dim
                a_input_index[reduce_dim] = i
                # Find corresponding positions to the indices
                output_pos = index_to_position(output_index, out_strides)
                a_pos = index_to_position(a_input_index, a_strides)
                # Calculate the value of the output with a function applied to the correct input
                ...
                if i == 0:
                    out[output_pos] = a_storage[a_pos]
                else:
                    out[output_pos] = fn(out[output_pos], a_storage[a_pos])
